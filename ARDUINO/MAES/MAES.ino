...
#define SERIAL_SETTING SERIAL_8E2
...
void setup()
{
	...
	SERVER_PORT.begin(SERIAL_RATE, SERIAL_SETTING);
#if DEBUG_ENABLED
	DEBUG_PORT.begin(SERIAL_RATE, SERIAL_SETTING);
	while (!DEBUG_PORT)
	{
...
void loop()
{
	...
	while (!intruction_allowed)
	{
    	...
    	DEBUG_PORT.println(compare_encrypt, DEC);
			...
			DEBUG_PORT.println(compare_testing, DEC);
		#endif
		*/

		if (compare_decrypt == 0)
		{
			SERVER_PORT.print(MSG_RECV);
			current_operation = M_DECRYPT;
			intruction_allowed = true;
		}
		else if (compare_encrypt == 0)
		{
			SERVER_PORT.print(MSG_RECV);
	...
	switch (current_operation)
	{
		...
		case M_ENCRYPT:
		{
			...
			//WAIT_STEP
			server_print_state(encrypt_hold);
			#if DEBUG_ENABLED
		...
		case M_DECRYPT:
		{
			...
			//WAIT_STEP
			server_print_state(decrypt_hold);
			#if DEBUG_ENABLED
				debug_print_state_as_string(decrypt_hold);
	...
	DEBUG_PORT.print("Operation Lasted: ");
	DEBUG_PORT.print(duration);
	DEBUG_PORT.print("ms\r\n");
#endif
