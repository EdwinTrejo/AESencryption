...
void BreakWordDown(uint8_t* ret_word, unsigned long word2);
void KeyExpansion(unsigned long* w, uint8_t *key);
void addKey(uint8_t* state, uint8_t* key);
void PackKey(uint8_t* key, unsigned long key_part1, unsigned long key_part2, unsigned long key_part3, unsigned long key_part4);
void ShiftRows(uint8_t* state);
void SubBytes(uint8_t* state);
void MixColumns(uint8_t* state);
uint8_t TimesThree(uint8_t num);
uint8_t TimesTwo(uint8_t num);
uint8_t* Encrypt(uint8_t state[16], uint8_t key[16], uint8_t *local_state);
...
int main(/*int argc, char** argv*/)
{
	...
	// uint32_t word[44] = { 0 };
	// //memcpy(word, KeyExpansion(key), 44 * sizeof(uint32_t));
	// for (int i = 0; i < 44; i++)
	// {
	// 	printf("%i: %X\n", i, word[i]);
	// }
	...
	// //addKey(&state, word[i]);
	// //memcpy(key, PackKey(word[4 * 0], word[4 * 0 + 1], word[4 * 0 + 2], word[4 * 0 + 3]), 16 * sizeof(uint8_t));
	// addKey(state, key);
	...
	// 	//XOR with key
	// 	i++;
	// 	//memcpy(key, PackKey(word[4 * i], word[4 * i + 1], word[4 * i + 2], word[4 * i + 3]), 16 * sizeof(uint8_t));
	// 	addKey(state, key);
	// }
}
...
uint8_t* Encrypt(uint8_t state[16], uint8_t key[16], uint8_t *local_state)
{
  ...
  //uint8_t local_state[16] = { 0x00 };
  //uint8_t local_key[16] = { 0x00 };

  //unsigned long *expanded_key = malloc(sizeof(unsigned long) * 44);

  unsigned long expanded_key[44];

  //uint8_t *local_state = malloc(sizeof(uint8_t) * 16);
  //uint8_t *local_key = malloc(sizeof(uint8_t) * 16);
  uint8_t local_key[16];

  //copy state
  memcpy(local_state, state, 16 * sizeof(uint8_t));
  memcpy(local_key, key, 16 * sizeof(uint8_t));
  //run key expansion
  KeyExpansion(expanded_key, local_key);

  //run first addkey
  //memcpy(local_key, PackKey(expanded_key[4 * 0], expanded_key[4 * 0 + 1], expanded_key[4 * 0 + 2], expanded_key[4 * 0 + 3]), 16 * sizeof(uint8_t));
  ...
  for (int i = 0; i < 10;)
  {
    ...
    //XOR with key
    i++;
    //memcpy(local_key, PackKey(expanded_key[4 * i], expanded_key[4 * i + 1], expanded_key[4 * i + 2], expanded_key[4 * i + 3]), 16 * sizeof(uint8_t));
...
void ShiftRows(uint8_t* state)
{
  ...
  //uint8_t *temp_state = malloc(sizeof(uint8_t) * 16);
  //top row
...
uint8_t TimesTwo(uint8_t num) {
  // ab = example of num, hex uint8_t

  // get eighth digit
  uint8_t eighth_digit = num >> 7;
...
void MixColumns(uint8_t* state)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    {
      int current = i * 4 + j;
      //uint8_t s0 = 0x02 * state[current];
      //uint8_t s1 = 0x03 * state[current + 1];
      //uint8_t a = s0 ^ s1;
      //uint8_t b = state[current + 2] ^ state[current + 3];
      //uint8_t c = a ^ b;
      switch (j)
      {
      case 0:
...
void PackKey(uint8_t* key, unsigned long key_part1, unsigned long key_part2, unsigned long key_part3, unsigned long key_part4)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    temp_key = key_part1;
    count--;
    uint8_t inv_shift_pos = 24 - (count * 8);
...
unsigned long SubWord(unsigned long word2)
{
  //sub byte
  uint8_t words[4];
  //uint8_t *words = malloc(sizeof(uint8_t) * 4);
  BreakWordDown(words, word2);
  unsigned long return_word = 0;
  uint8_t i, temp;
  for (i = 0; i < 4; i++) {
    switch (i) {
    case 0: { temp = getSBoxValue(words[3]); break; }
    case 1: { temp = getSBoxValue(words[2]); break; }
    case 2: { temp = getSBoxValue(words[1]); break; }
    case 3: { temp = getSBoxValue(words[0]); break; }
    }
